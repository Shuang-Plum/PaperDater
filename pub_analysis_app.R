             ...
             sidebarLayout(
               ...
               mainPanel(width=10,
                         ...
                         wellPanel(id="j.notification",style = "height:45px; font-size: 18px; padding: 8px 8px",
                                   tags$p(strong('Wait time is long for Journal Profiler, approx 20-50 min.'))),
                         hr(),
                         wellPanel(id="j.query",style = "overflow-y:scroll; height:100px; font-size: 20px; padding: 8px 8px",
                                   shinycssloaders::withSpinner(
                                     htmlOutput(outputId = 'journal.query'),
                                     type = 1, color = "#408000", size = 0.6,proxy.height=80)
                                   ),
...
server <- function(input, output, session) {
  ...
  output$download_key_data <- downloadHandler(
    ...
    content = function(file) { 
      write.csv(k.data.dl(), file, row.names = F) 
    }
  )
  ...
  output$download_mesh_plot <- downloadHandler(
    ...
    content = function(file) {
      ggsave(meshmatch.plot(), device='pdf', 
             filename = file, width =4200, height =y.max()*390,units='px')
    }
  )

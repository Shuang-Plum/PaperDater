             ...
             sidebarLayout(
               sidebarPanel(width = 2,
                            ...
                            textAreaInput(
                              ...
                              placeholder = 'fauci as [au] or fauci, anthony [au] or orcid 0000-0001-5027-4446 [auid]',
                              height = '100px'
                            ),
                            ...
                            hr(),
                            tags$p(tags$a("Author name format",href="https://pubmed.ncbi.nlm.nih.gov/help/#au",target="_blank"),
                                   HTML('&nbsp;'),": last name followed by a space and up to the first two initials followed by a space and a suffix abbreviation, if applicable, all without periods or a comma after the last name. Initials and suffixes may be omitted when searching."),
                            br(),
                            tags$p('Click here to download detailed supporting information'),
               ...
               mainPanel(width=10,
                         ...
                         wellPanel(id="a.query",style = "overflow-y:scroll; height:300px; font-size: 20px; padding: 8px 8px",
                                   shinycssloaders::withSpinner(
...
server <- function(input, output, session) {
  ...
  updateTextAreaInput(session, 'authorid')
  ...
  author.ris<-eventReactive(input$a.search, {
    ...
    author_search(input$authorid, input$a.starty, input$a.endy) 
  ...
  observeEvent(input$a.search, {
    ...
    output$author.query<-renderUI({
      ...
      validate(
        need(input$authorid, 'Please enter an Author Name or an Author Identifier (e.x. ORCID)') %then%
  ...
  observeEvent(input$a.search, {
    ...
    output$author.plot <- renderPlot({
      ...
      author_filter_plot(author.data(), input$a.min.count.forlabel, input$authorid)
  ...
  output$download_author_plot <- downloadHandler(
    ...
    filename = function() { paste0(input$authorid, '_author_plot.pdf') },
    content = function(file) {
      ggsave(author_filter_plot(author.data(), input$a.min.count.forlabel, input$authorid),
             device='pdf', filename = file, width =5350, height =2450,units='px')
    }
  )
  
  output$download_author_data <- downloadHandler(
    filename = function() { paste0(input$authorid, '_author_data.csv') },
    content = function(file) { 
